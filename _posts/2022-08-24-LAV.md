...
그러나 객체지향은 총알 역할을 하는 Prefab 오브젝트를 인스턴스화하기 시작하면서, 컴포넌트는 서스펜션 구현을 위해 '휠 콜라이더(Wheel Collider)'를 적용하면서부터 차차 이해되었다. 특히 객체지향의 경우 `Instantiate()` 함수를 활용하다 "틀과 붕어빵"이라는, 이전까지 깔끔하게 이해되지 않던 비유의 의미와 의도를 깨닫게 되면서 작은 감격을 느낀 순간이 있다. 말인즉, 좋은 객체지향설계를 위한 조건 중 하나가 적절한 '추상화'임을 납득했다.

<br>

# 블렌더

버텍스와 버텍스를 이어 에지를, 에지와 에지를 이어 페이스를 만들어 내 고유의 모델을 만들고 이를 유니티에 임포트하여 상호작용을 시킨다는 것은 정말로 신기한 경험이었다.  
