...
Chirpy라는 이름의 템플릿인데, 다른 템플릿 대체제도 많았지만 심미성과 추가기능 지원의 관점에서 Chirpy를 포기하고 싶지 않아 오기로 견뎌냈다. 구글링으로 정보를 찾아봐도 국내의 정보로는 해결이 힘든 부분이 있었으므로, Chirpy 템플릿 적용에 도움이 된 글과 방법을 정리하여 작성해 둔다.  
...
가장 먼저, 그리고 많이 만나게 되는 오류다. 해결하기 위해서는 Git Bash으로 블로그 파일이 위치한 경로로 이동한 뒤,
