...
`Assets\Plugins\Android\GoogleMobileAdsPlugin.androidlib`{: filepath }에 있는 `AndroidManifest.xml`와
`Assets\Plugins\Android`{: filepath }에 있는 `AndroidManifest.xml`의 

```HTML
<meta-data android:name="com.google.android.gms.ads.APPLICATION_ID" android:value="ca-app-pub-2168910631722247/1150412402"/>
```

android:value값이 일치하도록 수정해야 한다.

+ 애드몹 스크립트의 
```cs
#if UNITY_ANDROID
  private string _adUnitId = "ca-app-pub-2168910631722247/1150412402";
```
도 일치해야 함.
