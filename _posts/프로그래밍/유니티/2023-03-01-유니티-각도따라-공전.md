...
유니티는 공전을 위한 함수 `RotateAround()` 를 지원하지만 매개변수로 속력값을 받는데, 때문에 행성처럼 끝없이 이어지는 공전을 구현하고 싶은 것이 아니라면 사용하기에 불편함이 있다. 이는 각도값 θ와 길이 r이 주어질 때 `(cosθ * r, sinθ * r)`의 형식으로 좌표를 구하는 간략한 방법을 인용할 수 있다. 실질적인 코드로 변환하면, 아래와 같이 `(Mathf.Cos(mustFollowAngle * Mathf.Deg2Rad), Mathf.Sin(mustFollowAngle * Mathf.Deg2Rad)` 꼴이 된다.
