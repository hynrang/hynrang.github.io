...
![game-enter-third](/2023-12-13-cubic-survival-beta/game-enter-third.gif){: w="960" .shadow }
...
**[사진 취미](https://hynrng.github.io/posts/%EC%9E%84%EC%9D%B8%EB%85%84%EC%9D%98-%EC%82%AC%EC%A7%84/)**를 통해 느낀 점 중 하나는 그림이든 사진이든 영상이든, 2차원 형식을 공유한다면 화각(FOV)을 다루는 것이 가장 중요하다는 것입니다. 생각보다 많은 것을 표현할 수 있거든요.
...
예를 들어 위에서 차례차례 다룬 무기의 재장전, 플레이 씬으로의 전환 등등이 그렇습니다. 재장전 시에는 시야를 좁혀 무력감이 표현된다면 재미있겠다고 생각했고, 플레이가 시작될 때 역시 시야를 좁혀 긴장감을 줄 수 있지 않을까 하는 생각에 카메라가 보여주는 영역이 줄어들도록 만들었어요.

이외에 URP를 통해 블룸(Bloom) 포스트 프로세싱 효과도 주었습니다. 다른 포스트 프로세스 효과 중에 블러, 모션블러 정도도 시도는 해봤지만 결과물을 보니 너무 과하다는 생각이 들어 블룸 하나만 적용하게 되었네요.
...
**[유니티 에셋 스토어](https://assetstore.unity.com/ko)**에도 제가 원하는 효과음이 없어서, 대신 주로 **[Pixabay](https://pixabay.com/ko/sound-effects/)**에서 무료 오디오 효과를 얻어서 사용했습니다.
...
를 주로 이용했습니다. 제가 만들고 있는 게임이 그렇게 많은 효과음이 필요한 것 같지는 않아서


그중에 몇몇은 **[Audacity](https://www.audacityteam.org/)**를 이용해 편집한 뒤 사용했습니다.
...
통일성을 위해 어플 로고는 플레이버튼과 동일한 이미지로 넣었습니다. 이 프로젝트에 있어서 스토어 등록은 약간의 상징적인 의미가 있는 것이지, 이 게임으로 관심을 끌고 싶다 이런 생각은 없어서 직관성이 떨어지는 것은 감수하기로 했어요.
...
사실 등록 자체는 **[초기 개발 단계](https://hynrng.github.io/posts/Cubic-Survival-planning/)**에서 진행을 했는데요, 어플 등록 과정에는 어떤 절차가 있는지, 그런 절차를 모두 완료하면 정말로 어플이 올라가는지 등등 개발 도중 호기심에 등록한 거라, 등록 후 게임이  검색되는 것을 확인한 후에 바로 어플을 비활성화시켰었습니다.

그리고 반년 넘게 시간이 지나고 이제 슬슬 그만 만들고 싶기도 하고, 게임의 완성도가 처음보다 조금 봐줄만해졌다 싶어서 어플을 업데이트한 후 활성화하기로 했습니다. 등록중에 있어서는 어플명과 어플 설명을 작성했고, 앱 아이콘과 그래픽 이미지, 그리고 자체 스크린샷 3장을 업로드했습니다.

![play-store](/2023-12-13-cubic-survival-beta/play-store.png){: w="960" .shadow }
_우와 신기하다_

그래서 현재는 최종적으로 어플이 올라가있는 상태입니다.

## **홍보와 피드백**

홍보도 마찬가지로 생각을 해본 적이 없었는데 그래도 만든 것이 '게임'이니 플레이해주는 사람이 있으면 좋겠다는 생각이 들었습니다.

그런데 이 게임은 처음부터 플레이하라고 만들었다기보단, 토이 프로젝트로 도전했던 게 재미를 붙이다보니 스케일이 점점 커진 경우에 가깝다 보니 이걸 홍보하는게 맞나 싶기도 하고, 또 막상 제가 만든 것을 알리려니 부끄러움이 앞서더라구요.

![reddit](/2023-12-13-cubic-survival-beta/reddit.png){: w="960" .shadow }

그래도 용기를 내서 해외 [Unity2D 서브레딧](https://www.reddit.com/r/Unity2D/)에 짧은 글을 올렸습니다. 한 100명 봐주면 정말 감사하겠다 하는 마음가짐으로 글을 올렸는데, 일주일만에 조회수가 2만을 넘더니 한달 정도가 지나자 무려 10만에 가까운 분들이 관심을 가져주셨습니다.

![reddit-feedback](/2023-12-13-cubic-survival-beta/reddit-feedback.png){: w="960" .shadow }

그중에 몇몇 분은 정말 감사하게도 직접 플레이하셔서 위와 같이 상세한 피드백까지 주시더라구요. 주로 나온 의견은

- 조이스틱의 위치가 수정 불가능한 채로 고정되어 있어 불편하다.
- 블룸(Bloom)이 과한 것 같다.
- 타 게임과 비슷해 보인다.

이 정도였습니다. 의외로 다른 게임과 비슷해보인다는 의견이 있었는데, 오히려 제가 의도한 대로 잘 나온 것 같아 기분이 좋았습니다. 다른 게임과 같이 보일 정도로 완성도는 있어보였다는 것이니까요.
...
솔직히 말하자면 근본적으로.. 플레이스토어에 버젓이 올라가있는 제 게임을 보면서 "이건 분명 플레이하라고 만든 것이기는 하지만 또 한 편으로는 플레이하라고 만든 게 아닌 것 같다"라는 생각이 들었습니다. 게임의 기획 자체가 부실하기도 했고, 무엇보다 이것저것 신기한 기능들을 하나하나 사용해볼 수 있는 무언가에 가까던 것 같거든요.

그래도 만드는 과정이 너무 재미있었어서 다음에 조만간 유니티로 새 프로젝트를 진행하지 않을까 싶습니다. 처음이 어렵지, 두번이 어려울 것 같지 않기도 하구요.