...
아무거나
![LAVAD_main2](/assets/img/2022-08-24-LAV/LAVAD_main2.png)써보는중
...
![LAVAD_modeling](/assets/img/2022-08-24-LAV/LAVAD_modeling.png){: width="972" height="589" .left}
프로젝트의 주제는 **LAV-AD**라는, 군용 차량입니다. 굳이 상세히 하자면 대공장갑차량이죠. 많고 많은 주제 중에 하필 LAV-AD인 이유는 두 가지가 있습니다. 하나는 이전에 **배틀필드 4**라는 게임에서 이 차종을 정말 재미있게 플레이한 기억이 있기 때문이고, 다른 하나는 제가 알고 있는 차량 중에서는 가장 기하학적인 형태를 지니고 있어 모델링이 할만해보였기 때문입니다.

![LAVAD_modeling](/assets/img/2022-08-24-LAV/LAVAD_modeling.png){: width="972" height="589" .w-50 .left}
프로젝트의 주제는 **LAV-AD**라는, 군용 차량입니다. 굳이 상세히 하자면 대공장갑차량이죠. 많고 많은 주제 중에 하필 LAV-AD인 이유는 두 가지가 있습니다. 하나는 이전에 **배틀필드 4**라는 게임에서 이 차종을 정말 재미있게 플레이한 기억이 있기 때문이고, 다른 하나는 제가 알고 있는 차량 중에서는 가장 기하학적인 형태를 지니고 있어 모델링이 할만해보였기 때문입니다.
...
인터넷에서 모델을 구할 생각도 해봤지만, 찾다보니 무료모델은 품질이 조악했고, 유료모델은 두 자릿수의 달러를 요구하고 있었습니다. 전면 설계도와 같은 자료를 모으고 블렌더로 모델링을 하기 시작했습니다. 3D 모델 파일을 인터넷에서 구할 수는 있지만, 무료 모델은 퀄리티가 낮았고 쓸만한 모델은 두 자릿수의 달러를 요구했기 때문에 과감히 도전했는데, 버텍스와 버텍스를 이어 에지를, 에지와 에지를 이어 페이스를 만들어 나만의 3D 모델을 만들고 이를 게임 시스템 구성원으로 임포트하는 것은 이전에 겪지 못했던 신기한 경험이었습니다.
