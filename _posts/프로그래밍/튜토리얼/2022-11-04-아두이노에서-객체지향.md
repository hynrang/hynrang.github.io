...
개인적으로 아두이노를 별로 좋아하는 편은 아닙니다. 하드웨어의 오류를 발견하기 쉽지 않기 때문인데, 이번에 아두이노로 물건을 만들어야 하는 상황에 놓이게 되었습니다.

이왕 하는거 새로운거 도전해서 뭐라도 배워가자 하는 마음에 빈약한 지식으로 객체지향 설계를 시도했습니다. 아두이노는 C++을 기반으로 동작하는데, 주로 사용하는 C#이나 파이썬과는 특성이 달라 조금 생소했습니다. 주어진 시간도 넉넉치 않아 어려움이 있었지만 잘 동작하는 모습까지 확인하고 나니 뿌듯합니다. 혹시라도 나중에 C++을 다루게 된다면 도움이 되면 좋겠습니다.
